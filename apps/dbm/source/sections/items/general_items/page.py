...
from sections.items.general_items.record_list import GeneralItemList
...
class GeneralItemsPage(tk.Frame):
    def __init__(self, parent, database):
        super().__init__(parent, bg=APP_BACKGROUND)
        bind_theme(self, background="APP_BACKGROUND")

        self.database = database
        self.controller = GeneralItemController(database)
        self.records = []
        ...
        self.record_list = GeneralItemList(
            self.list_card,
    ...
    def new_record(self):
        ...
        self.status_value.set("Creating a new general item")
    ...
    def save_record(self):
        ...
        if not record_values["name"]:
            messagebox.showerror(
                "Name required",
                "A general item must have a name.",
                parent=self,
            )
            return False
    ...
    def delete_record(self):
        if self.current_record_id is None:
            ...
            self.form_dirty = False
            self.toolbar.set_record_state(dirty=False, has_record=False)
            self.status_value.set("New general item cleared")
            return

        record = self.controller.get_record(self.current_record_id)
        record_name = record.get("name", "this general item")
    ...
    def confirm_unsaved_changes(self):
        ...
        save_choice = messagebox.askyesnocancel(
            "Unsaved general item changes",
            "Save changes to this general item before continuing?",
            parent=self,
        )
