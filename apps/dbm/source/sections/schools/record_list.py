...
class SchoolList(tk.Frame):
    ...
    @staticmethod
    def build_display_text(record, repeated_name=False):
        school_name = (
            str(record.get("name", "")).strip() or "Unnamed school"
        )
        location = (
        ...
        if repeated_name:
            school_name = f"{school_name} — {record.get('record_id', '')}"
    ...
    def set_records(self, records, selected_record_id=None):
        ...
        name_counts = Counter(
            str(record.get("name", "")).casefold() for record in records
        )
        self.labels_by_id = {
            record["record_id"]: self.build_display_text(
                record,
                repeated_name=(
                    name_counts[str(record.get("name", "")).casefold()] > 1
                ),
            )
    ...
    def rebuild_visible_list(self):
        ...
        if visible_count == total_count:
            self.count_label.configure(text=f"{total_count} schools")
        else:
            self.count_label.configure(
                text=f"{visible_count} of {total_count} schools"
            )
