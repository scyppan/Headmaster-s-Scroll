...
from sections.wands.wand_makers.record_form import WandMakerForm
from sections.wands.wand_makers.record_list import WandMakerList
...
class WandMakersPage(tk.Frame):
    def __init__(self, parent, database):
        super().__init__(parent, bg=APP_BACKGROUND)
        bind_theme(self, background="APP_BACKGROUND")

        self.database = database
        self.controller = WandMakerController(database)
        self.records = []
        ...
        self.record_list = WandMakerList(
            self.list_card,
        ...
        self.record_form = WandMakerForm(
            self.form_card,
    ...
    def load_record(self, record_id):
        ...
        self.status_value.set(f"Loaded {record.get('name', 'maker')}")
    ...
    def new_record(self):
        ...
        self.status_value.set("Creating a new wand maker")
    ...
    def delete_record(self):
        if self.current_record_id is None:
            ...
            self.form_dirty = False
            self.toolbar.set_record_state(dirty=False, has_record=False)
            self.status_value.set("New wand maker cleared")
            return

        record = self.controller.get_record(self.current_record_id)
        record_name = record.get("name", "this wand maker")
    ...
    def confirm_unsaved_changes(self):
        ...
        save_choice = messagebox.askyesnocancel(
            "Unsaved wand maker changes",
            "Save changes to this wand maker before continuing?",
            parent=self,
        )
