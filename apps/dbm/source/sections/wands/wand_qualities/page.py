...
from sections.wands.wand_qualities.record_form import WandQualityForm
from sections.wands.wand_qualities.record_list import WandQualityList
...
class WandQualitiesPage(tk.Frame):
    def __init__(self, parent, database):
        super().__init__(parent, bg=APP_BACKGROUND)
        bind_theme(self, background="APP_BACKGROUND")

        self.database = database
        self.controller = WandQualityController(database)
        self.records = []
        ...
        self.record_list = WandQualityList(
            self.list_card,
        ...
        self.record_form = WandQualityForm(
            self.form_card,
    ...
    def load_record(self, record_id):
        ...
        self.status_value.set(f"Loaded {record.get('name', 'quality')}")
    ...
    def new_record(self):
        ...
        self.status_value.set("Creating a new wand quality")
    ...
    def save_record(self):
        ...
        if not record_values["name"]:
            messagebox.showerror(
                "Name required",
                "A wand quality must have a name.",
                parent=self,
            )
            return False
    ...
    def delete_record(self):
        if self.current_record_id is None:
            ...
            self.form_dirty = False
            self.toolbar.set_record_state(dirty=False, has_record=False)
            self.status_value.set("New wand quality cleared")
            return

        record = self.controller.get_record(self.current_record_id)
        record_name = record.get("name", "this wand quality")
    ...
    def confirm_unsaved_changes(self):
        ...
        save_choice = messagebox.askyesnocancel(
            "Unsaved wand quality changes",
            "Save changes to this wand quality before continuing?",
            parent=self,
        )
